...
class DoctorController extends AbstractController
{
    ...
    public function form(
        ...
        // Get the user for edit mode
        $user = $isEdit ? $doctor->getClient() : null;

        // Create the form first
        $form = $this->createForm(DoctorType::class, $doctor);

        // If editing and user exists, pre-fill the form data
        if ($isEdit && $user) {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            // Handle password - update if provided in edit mode, or set for new doctors
            $plainPassword = $form->get('plainPassword')->getData();
            if ($plainPassword) {
                $user->setPassword($hasher->hashPassword($user, $plainPassword));
            } elseif (!$isEdit) {
                // For new doctors, password is required (validation should handle this)
                $this->addFlash('error', 'Password is required for new doctors.');

...
class DoctorDashboardController extends AbstractController
{
    ...
    public function dashboard(AppointmentRepository $apptRepo): Response
    {
        ...
        // FIX: Create adjusted time for prescription comparison
        $nowAdjusted = new \DateTime('now');
        $nowAdjusted->modify('+1 hour'); // Add 1 hour for timezone adjustment
        ...
        return $this->render('dashboard/doctor.html.twig', [
            ...
            'stats' => $stats,
            'doctor' => $doctor,
            'nowAdjusted' => $nowAdjusted, // ADD THIS LINE - pass to Twig
        ]);
    }
    ...
    public function completeAppointment(Appointment $appointment, EntityManagerInterface $em): JsonResponse
    {
        ...
        // Check if appointment has ended with +1 hour adjustment
        $now = new \DateTime();
        $now->modify('+1 hour'); // Add 1 hour adjustment

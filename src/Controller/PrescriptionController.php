...
class PrescriptionController extends AbstractController
{
    ...
    public function upload(
        ...
        $now = new \DateTime();
        $now->modify('+1 hour'); // Add this line
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($uploadedFile) {
                ...
                // FIX: Use getClientOriginalExtension() instead of guessExtension()
                $originalExtension = $uploadedFile->getClientOriginalExtension();
                $newFilename = $safeFilename.'-'.uniqid().'.'.$originalExtension;

                // FIX: Manual file extension validation
                $allowedExtensions = ['pdf', 'jpg', 'jpeg', 'png'];
                if (!in_array(strtolower($originalExtension), $allowedExtensions)) {
                    $this->addFlash('error', 'Invalid file type. Please upload PDF, JPG, JPEG, or PNG files only.');

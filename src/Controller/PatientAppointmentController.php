...
class PatientAppointmentController extends AbstractController
{
    ...
    public function cancelAppointment(Appointment $appointment, EntityManagerInterface $em, AvailabilityRepository $availabilityRepo): JsonResponse
    {
        ...
        // Check if appointment can be cancelled (should not be already cancelled or completed)
        if ($appointment->getStatus() === 'Cancelled') {
            return $this->json(['success' => false, 'message' => 'Appointment is already cancelled']);
        }

        // Try to find the availability slot
        $startDateTime = $appointment->getStartDateTime();

        // Method 1: Look for availability with the exact date/time
        $availability = $availabilityRepo->findOneBy([
            'doctor' => $appointment->getDoctor(),
            'isAvailable' => false
        ]);

        // Method 2: If not found by exact time, look for any availability on the same date
        if (!$availability) {
            $availabilities = $availabilityRepo->findBy([
                'doctor' => $appointment->getDoctor(),
                'isAvailable' => false
            ...
            // Find the one that matches the appointment time
            foreach ($availabilities as $avail) {
                if ($avail->getDate() && $avail->getDate()->format('Y-m-d') === $startDateTime->format('Y-m-d')) {
                    $availability = $avail;
                    break;
                }
            }
        ...
        // If we found the availability, mark it as available again
        if ($availability) {
        ...
        return $this->json(['success' => true, 'message' => 'Appointment cancelled successfully']);
    }
